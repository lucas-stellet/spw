# Thoughtbox -> Oráculo: O que aproveitar em funcionalidade, design e filosofia

## Objetivo deste documento

Este documento descreve, de forma detalhada, o que o projeto Thoughtbox oferece hoje e como essas ideias podem ser adaptadas para o Oráculo sem quebrar os principios atuais do kit (thin orchestrator, subagent-first, aprovacoes MCP, handoff por arquivo e checkpoints por wave).
...
O Oráculo ja e forte em orquestracao, guardrails e estrutura de artefatos por fase. O Thoughtbox complementa isso com tres forcas principais que o Oráculo ainda pode explorar melhor:
...
Se o Oráculo incorporar esses tres pilares, ele deixa de ser apenas um "motor de fluxo" e passa a ser tambem um "sistema observavel e evolutivo" para engenharia assistida por agentes.
...
Por que isso importa para o Oráculo:

- hoje o Oráculo guarda briefs/reports/status por subagente, mas nao tem uma camada unica de "timeline executiva" do processo
...
- o Oráculo hoje e "forte em estrutura" e "fraco em telemetria de runtime"
...
- Oráculo ja tem gates fortes (aproval, checkpoint, exec policy), mas pode evoluir para "policy por estagio de comando"
...
- Oráculo valida muito bem formato e fluxo, mas pode validar melhor "substancia"
...
- Oráculo tem post-mortem por spec, mas nao tem scorecard acumulado de runtime do sistema
- sem serie historica, fica dificil otimizar estrategia de waves e subagentes

---

## O que ja existe no Oráculo e nao deve ser perdido
...
## Propostas concretas para trazer ao Oráculo

## Proposta A - Event Ledger do workflow (prioridade P0)

### O que e

Criar um log append-only por spec para eventos de runtime do Oráculo, por exemplo:
...
## Proposta B - Oráculo Observatory (prioridade P1)
...
- primeiro TUI/CLI (`oraculo observability`)
...
## Proposta C - Behavioral Contracts para comandos Oráculo (prioridade P1)

### O que e

Adicionar uma camada de testes de comportamento do proprio fluxo do Oráculo, alem dos testes estruturais.
...
## Diretriz de design para adocao no Oráculo
...
- Criar matriz de behavioral contracts do Oráculo
...
- Oráculo vira um produto de observabilidade em vez de kit de workflow
...
O Thoughtbox nao precisa ser replicado no Oráculo para gerar valor.
...
Com isso, o Oráculo mantem sua identidade (thin orchestrator + gates MCP + file-first) e ganha uma camada que hoje falta: visibilidade e evolucao sistematica do proprio processo.
