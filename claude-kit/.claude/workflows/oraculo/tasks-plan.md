...
5. `oraculo tools dispatch-read-status decision-aggregator --run-dir <audit_dir>`
...
7. If BLOCKED:
   ...
   b. If allowed: `oraculo tools audit-iteration advance --run-dir <RUN_DIR> --type inline-audit --result blocked`, re-dispatch tasks-writer with aggregator report path as feedback, then repeat from step 2 with next iteration
   c. If exhausted: STOP, recommend `oraculo:tasks-check <spec-name>`
...
- Recommend next command: `oraculo:tasks-check <spec-name>` (for re-validation or CI gate).
