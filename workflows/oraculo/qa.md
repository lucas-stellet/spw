...
- @.claude/workflows/oraculo/shared/dispatch-inline-audit.md
</shared_policies>
...
Follow @.claude/workflows/oraculo/shared/dispatch-inline-audit.md.

1. `oraculo tools audit-iteration start --run-dir <RUN_DIR> --type inline-audit`
2. `oraculo tools dispatch-init-audit --run-dir <RUN_DIR> --type inline-audit --iteration 1`
...
5. `oraculo tools dispatch-read-status qa-check-aggregator --run-dir <audit_dir>`
6. If PASS → proceed to artifact verification below
7. If BLOCKED:
   a. `oraculo tools audit-iteration check --run-dir <RUN_DIR> --type inline-audit`
   b. If allowed: `oraculo tools audit-iteration advance --run-dir <RUN_DIR> --type inline-audit --result blocked`, re-dispatch qa-plan-synthesizer with aggregator report path as feedback, then repeat from step 2 with next iteration
   c. If exhausted: STOP, recommend `oraculo:qa-check <spec-name>`
...
- Recommend next command: `oraculo:qa-check <spec-name>` (for re-validation or CI gate).
