...
```
spw tools audit-iteration start --run-dir <parent-run> \
...
```
spw tools dispatch-init-audit --run-dir <parent-run> \
...
```
spw tools audit-iteration check --run-dir <parent-run> \
...
```
spw tools audit-iteration advance --run-dir <parent-run> \
...
On PASS: `spw tools wave-update --spec X --wave NN --status pass --tasks ...`
On BLOCKED: max 1 retry (re-dispatch task-implementer to fix missing artifacts),
then recommend standalone `/spw:checkpoint`.

## Fallback to Standalone Commands

When inline audit exhausts its iterations:
- `tasks-plan` → recommend `spw:tasks-check <spec-name>`
- `qa` → recommend `spw:qa-check <spec-name>`
- `exec` → recommend `spw:checkpoint <spec-name>`
