...
Follow the pattern defined in `@.claude/workflows/oraculo/shared/dispatch-inline-audit.md`.

**Quick reference:**

a) Initialize iteration tracking:
```
oraculo tools audit-iteration start --run-dir <RUN_DIR> --type <inline-audit|inline-checkpoint>
```

b) Create nested audit directory:
```
oraculo tools dispatch-init-audit --run-dir <RUN_DIR> --type <type> [--iteration N]
```
...
```
oraculo tools audit-iteration check --run-dir <RUN_DIR> --type <type>
oraculo tools audit-iteration advance --run-dir <RUN_DIR> --type <type> --result blocked
```
...
See `@.claude/workflows/oraculo/shared/file-handoff.md` section 6 for the
