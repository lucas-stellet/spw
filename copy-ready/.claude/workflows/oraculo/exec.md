...
4. Run deterministic wave state resolution:
   ```
   oraculo tools wave-status --spec <spec-name>
   ```
...
1. At end of batch, run inline checkpoint before recommending standalone checkpoint:
   ...
   a. Initialize:
      ```
      oraculo tools dispatch-init-audit --run-dir <current-run> --type inline-checkpoint
      ```
   b. Dispatch `evidence-collector`, `traceability-judge`, `release-gate-decider` inside the audit dir.
      See `@.claude/workflows/oraculo/shared/dispatch-inline-audit.md` for the full pattern.
   c. On PASS:
      ```
      oraculo tools wave-update --spec <spec-name> --wave <NN> --status pass --tasks <ids>
      ```
   d. On BLOCKED: max 1 retry (re-dispatch task-implementer to fix missing artifacts),
      then recommend standalone `/oraculo:checkpoint <spec-name>` in a new session.

2. If inline checkpoint was not run or returned BLOCKED after retries, instruct the user:
   "Wave complete. Run `/oraculo:checkpoint <spec-name>` in a new session (`/clear` first or new Claude Code session)."
...
1. Mark task in-progress:
   ```
   oraculo tools task-mark --spec <spec-name> --task-id <N> --status in-progress
   ```
2. Dispatch `task-implementer` (mandatory, even when single-task batch).
   - Subagent MUST run self-check before reporting pass (see `@.claude/workflows/oraculo/shared/file-handoff.md` section 6):
     ```
...
6. If all gates pass:
   - Mark task done:
     ```
     oraculo tools task-mark --spec <spec-name> --task-id <N> --status done
     ```
...
- Implementation logs are registered via `oraculo tools impl-log register` during execution and verified inline via `oraculo tools verify-task`. Missing logs block the task before reviewers run.
